...
model = AutoModelForSequenceClassification.from_pretrained(model_name).to_device(device)
query_file_name = "queries_100.tsv"  # for the small version, use queries_100.tsv
...
def generate_list_with_scores():
    ...
    if not os.path.isfile("datasets/dataset_list.pickle"):
        dataset_list = []
        col_names = ["id", "text"]
        df_queries = pd.read_csv(
            f"/data/{query_file_name}", names=col_names, delimiter="\t"
        )
        ...
        with open("datasets/dataset_list.pickle", "wb") as f:
            pickle.dump(dataset_list, f)
    else:
        with open("datasets/dataset_list.pickle", "rb") as input_file:
...
def generate_dataset():
    ...
    with open(
        "data/triples.train.small.tsv", "w", encoding="utf8", newline=""
    ) as tsv_file:
